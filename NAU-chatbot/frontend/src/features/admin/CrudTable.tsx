...
function referenceValue(value: unknown, column: EntityColumn, references: ReferenceData): string {
...
function cellValue(entity: AcademicEntity, column: EntityColumn, references: ReferenceData): string {
  ...
  if (column.format === "reference") return referenceValue(value, column, references);
...
export function CrudTable({ config, entities, loading, references, onEdit, onToggle, onDelete }: CrudTableProps) {
  ...
  if (!loading && entities.length === 0) {
    return (
      <div className="empty-state empty-state--compact">
        ...
        <p>Modifiez les filtres ou créez un nouveau {config.singular}.</p>
      </div>
    );
  }

  return (
    <div className={`table-wrap ${loading ? "table-wrap--loading" : ""}`} aria-busy={loading}>
      <table className="data-table">
        ...
        <thead>
          <tr>
            ...
            <th scope="col">État et source</th>
            ...
            return (
              <tr key={entity.id}>
                {config.columns.map((column, index) => (
                  <td data-label={column.label} key={column.key} className={index === 0 ? "data-table__primary" : undefined}>
